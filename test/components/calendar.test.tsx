...
describe("Calendar Component", () => {
  ...
  describe("Custom Props", () => {
    it("accepts and displays custom currentDate", () => {
      ...
      const { container } = render(() => 
        <Calendar currentDate={customDate} />
      );
    ...
    it("accepts custom dateFormat prop", () => {
      ...
      const dateButton = getByText("15");
      expect(dateButton).toHaveAttribute("aria-label", expect.stringMatching(/\d{2}\/\d{2}\/\d{4}/));
    });

    it("handles disabled dates", () => {
      const testDate = new Date(2024, 0, 15); // January 15, 2024
      const { getByText } = render(() => 
        <Calendar disabledDates={[testDate]} currentDate={new Date(2024, 0, 1)} />
      );
